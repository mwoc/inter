...
(function (root, factory) {
    ...
    var inter = {
        ...
        makeUnitRenderer: function (unit) {
            var that = this,
                quantityRenderers = {};
            for (var quantity in that.unitPatterns[unit]) {
                if (that.unitPatterns[unit].hasOwnProperty(quantity)) {
                    quantityRenderers[quantity] = that.makePatternRenderer(pattern);
                }
            }
            return function (n) {
        ...
        renderCurrencyLong: function (value, currencyId, numberSystemId) {
            var currency = inter.getCurrency(currencyId),
                currencyString = currency ? this.trQuantity(currency, value) : null;
            ...
            return inter.renderNumber(value, this.numbers[numberSystemId].formats.currency.default, numberSystemId, currencyString);
        },

        renderCurrencyShort: function (value, currencyId, numberSystemId) {
            var currency = inter.getCurrency(currencyId),
                currencyString = currency ? currency.symbol : null;
